...
st.title("🚦 IntelliSignal AI with Emergency Priority")
...
def process_lane(frame, lane_idx, active_status):
    ...
    if has_amb:
        ...
        if start_times[lane_idx] is not None:
            ...
            if amb_gone_timer[lane_idx] == 0:
                amb_gone_timer[lane_idx] = current_now
    ...
    status_msg = (
        "PRIORITY GRANTED"
        if emergency_lane == lane_idx
        else f"VERIFYING AMBULANCE: {min(elapsed,5)}/5s"
    )
...
for i in range(4):
    ...
    if ret:
        ...
        frames[i] = np.zeros((360, 640, 3), dtype=np.uint8)
...
while run:
    ...
    frame_placeholder.image(grid, channels="BGR", use_container_width=True)
